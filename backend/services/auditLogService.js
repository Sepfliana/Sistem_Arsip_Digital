...
const createAuditLog = async (
    ...
    try {
        ...
        const hashSebelumnya =
            lastLog.rows.length > 0
                ? lastLog.rows[0].hash_entri
                : null;
        ...
        try {
            ...
            if (aiResponse.data?.status === "ANOMALY") {
                ...
                const threshold = Number(aiResponse.data.threshold || 0);
                const tingkatRisiko = threshold > 0 && score >= threshold * 1.5
                    ? "TINGGI"
                    : "SEDANG";
    ...
    for (const row of result.rows) {
        if (row.hash_sebelumnya !== previousHash) {
            return {
                valid: false,
                brokenAt: row.id,
                reason: "Hash sebelumnya tidak sesuai"
            };
        }
        ...
        if (row.hash_entri !== expectedHash) {
            return {
                valid: false,
                brokenAt: row.id,
                reason: "Hash entri tidak sesuai"
            };
        }
