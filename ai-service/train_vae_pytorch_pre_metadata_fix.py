...
def metrics(scores: np.ndarray, labels: np.ndarray, threshold: float) -> dict[str, Any]:
    ...
    tp = int((prediction & labels).sum()); tn = int((~prediction & ~labels).sum())
    fp = int((prediction & ~labels).sum()); fn = int((~prediction & labels).sum())
...
def distribution(scores: np.ndarray, feature_errors: np.ndarray) -> dict[str, Any]:
    ...
    return {
        "n": int(len(scores)),
        "score": {"min": float(scores.min()), "max": float(scores.max()), "mean": float(scores.mean()), "median": float(np.median(scores)), "std": float(scores.std()), "p95": float(np.percentile(scores, 95))},
...
def evaluate(data: dict[str, np.ndarray], labels: dict[str, pd.DataFrame], threshold: float) -> dict[str, Any]:
    ...
    for split in ("validation", "test"):
        ...
        for activity, rows in frame.groupby("activity").groups.items():
            ...
            activities[str(activity)] = {"n": int(mask.sum()), "score_mean": float(scores[mask].mean()), "flag_rate": float((scores[mask] > threshold).mean()), "anomaly_rate": float(anomaly_mask[mask].mean())}
        result["splits"][split] = {
            ...
            "time_breakdown": {"working_hours_08_15": group_distribution(scores, errors, working_mask), "outside_working_hours": group_distribution(scores, errors, ~working_mask)},
            "ip_breakdown": {"localhost_loopback": group_distribution(scores, errors, localhost_mask), "non_localhost": group_distribution(scores, errors, ~localhost_mask), "localhost_rows_present": int(localhost_mask.sum())},
...
def main() -> int:
    ...
    loader = DataLoader(TensorDataset(torch.from_numpy(data["train"])), batch_size=min(int(MODEL_CONFIG["batch_size"]), len(data["train"])), shuffle=True, generator=torch.Generator().manual_seed(SEED))
    history: list[dict[str, float | int]] = []
    best: dict[str, Any] = {"validation_loss": float("inf"), "epoch": 0, "state": None}
    for epoch in range(1, int(MODEL_CONFIG["epochs"]) + 1):
        model.train()
        total_sum = recon_sum = kl_sum = 0.0; samples = 0
        for (batch,) in loader:
            optimizer.zero_grad()
            total, reconstruction, kl = vae_losses(model, batch, epoch)
            total.backward(); optimizer.step()
            count = batch.size(0); samples += count
            total_sum += float(total.item()) * count; recon_sum += float(reconstruction.item()) * count; kl_sum += float(kl.item()) * count
        validation_loss, validation_recon, validation_kl = validation_losses(model, validation_normal, epoch)
        item = {"epoch": epoch, "train_loss": total_sum / samples, "train_reconstruction_loss": recon_sum / samples, "train_kl_loss": kl_sum / samples, "validation_normal_loss": validation_loss, "validation_normal_reconstruction_loss": validation_recon, "validation_normal_kl_loss": validation_kl, "kl_capacity": kl_capacity(epoch)}
        history.append(item)
    ...
    # P95 train-normal is selected before any test evaluation.  The mean+3σ
    # number is recorded only to make the historic documentation conflict auditable.
    with torch.no_grad():
    ...
    metadata = {
        "status": "TRAINED_AND_EVALUATED_PENDING_FINAL_AUDIT", "created_at": datetime.now(timezone.utc).isoformat(),
        "feature_order": list(FEATURE_COLUMNS), "architecture": MODEL_CONFIG,
        "parameter_count": int(sum(parameter.numel() for parameter in model.parameters())),
        "seed": {"python": SEED, "numpy": SEED, "torch": SEED},
        "training": {"train_rows": int(len(data["train"])), "train_anomaly_rows": 0, "validation_normal_rows": int(len(validation_normal)), "test_rows_not_used_for_training_or_threshold": int(len(data["test"])), "best_epoch_by_validation_normal_loss": int(best["epoch"]), "best_validation_normal_loss": float(best["validation_loss"]), "epochs_run": len(history)},
        ...
        "artifacts_sha256": {"model": sha256_of(MODEL_PATH), "config": sha256_of(MODEL_CONFIG_PATH), "threshold": sha256_of(THRESHOLD_PATH), "evaluation": sha256_of(EVALUATION_PATH), "history": sha256_of(TRAINING_HISTORY_PATH)},
